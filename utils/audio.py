...
def smoothing(wav, win = 0):
	wav = np.asarray(wav, dtype = np.float32)

	if win != 0:
		wav = inv_preemphasis(wav, 0.70)
		
		i = int(win / 2)
		mask = np.ones(win)
		smooth_wav = signal.convolve(wav, mask)
		smooth_wav = smooth_wav[i:-i]
		smooth_wav /= win
		wav = smooth_wav
	
	wav /= (np.max(np.abs(wav)) * 1.0)
...
def save_wav(wav, path, sr, win = 0):
	#wav *= 32767 / max(0.01, np.max(np.abs(wav)))
	wav = smoothing(wav, win)
...
def melspectrogram(wav, hparams):
	...
	return S
	'''
	D = librosa.feature.melspectrogram(preemphasis(wav, hparams.preemphasis, hparams.preemphasize), sr=hparams.sample_rate, n_fft=hparams.n_fft, hop_length=hparams.hop_size, n_mels=80,
                                                     fmin=hparams.fmin, fmax=hparams.fmax).T

	S = _amp_to_db(D.T, hparams) - hparams.ref_level_db
