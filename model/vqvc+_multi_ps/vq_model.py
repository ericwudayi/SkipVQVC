...
class PixelShuffle(nn.Module):
    ...
    def forward(self, input):
        ...
        c_out = int(input.shape[1] // self.upscale_factor)
        w_new = int(input.shape[2] * self.upscale_factor)

        return input.reshape(n, c_out, w_new)
...
class VC_MODEL(nn.Module):
    def __init__(
        ...
        gnorm = []
        for i in range(3):
            gnorm += [nn.GroupNorm(2, in_channel//(2**(i+1)))]
        self.gnorm = nn.ModuleList(gnorm)
        
        self.ps_down = PixelShuffle(1)
        self.ps_up = PixelShuffle(1)
    ...
    def forward(self, input):
        enc_b, sp_embed, diff, _ = self.encode(input)
        dec_1= self.dec(enc_b, sp_embed)
        ...
        dec_2 = self.dec(enc_b, sp_shuffle)
    ...
    def encode(self, input):
        ...
        for i, (enc_block, gn, quant) in enumerate(zip(self.enc, self.gnorm, self.quantize)):
            ...
            q_after, diff, index= quant(self.ps_down(x_).permute(0,2,1))
            q_after = q_after.permute(0,2,1)
            q_after = self.ps_up(q_after)
            ...
            diff_total += diff
            index_list += [index]
        return q_after_block, sp_embedding_block, diff_total, index_list
